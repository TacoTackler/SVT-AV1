...
typedef void (*av1_inv_txfm_highbd_func)(const int32_t *coeff,
                                         uint16_t *output_r, int32_t stride_r,
                                         uint16_t *output_w, int32_t stride_w,
                                         TxType tx_type, int32_t bd);
typedef void (*av1_inv_txfm2d_highbd_rect_func)(
    const int32_t *input, uint16_t *output_r, int32_t stride_r,
    uint16_t *output_w, int32_t stride_w, TxType tx_type, TxSize tx_size,
    int32_t eob, int32_t bd);
av1_inv_txfm_highbd_func av1_inv_txfm_highbd_func_ptr_array_base[3] = {
    eb_av1_inv_txfm2d_add_16x16_avx2,
    eb_av1_inv_txfm2d_add_32x32_avx2,
    eb_av1_inv_txfm2d_add_64x64_sse4_1};
av1_inv_txfm_highbd_func av1_inv_txfm_highbd_func_ptr_array_opt[3] = {
    eb_av1_inv_txfm2d_add_16x16_avx512,
    eb_av1_inv_txfm2d_add_32x32_avx512,
    eb_av1_inv_txfm2d_add_64x64_avx512};
av1_inv_txfm2d_highbd_rect_func
    av1_inv_txfm_highbd_rect_func_ptr_array_base[6] = {
...
av1_inv_txfm2d_highbd_rect_func av1_inv_txfm_highbd_rect_func_ptr_array_opt[6] =
...
static void init_data(int32_t **input, int32_t **input_opt,
                      int32_t *input_stride) {
    *input_stride = eb_create_random_aligned_stride(MAX_SB_SIZE, 64);
    TEST_ALLIGN_MALLOC(
        int32_t *, *input, sizeof(int32_t) * MAX_SB_SIZE * *input_stride);
    TEST_ALLIGN_MALLOC(
        int32_t *, *input_opt, sizeof(int32_t) * MAX_SB_SIZE * *input_stride);
...
static void init_data_with_max(int32_t **input, int32_t **input_opt,
                               int32_t *input_stride) {
    *input_stride = eb_create_random_aligned_stride(MAX_SB_SIZE, 64);
    TEST_ALLIGN_MALLOC(
        int32_t *, *input, sizeof(int32_t) * MAX_SB_SIZE * *input_stride);
...
static void init_output_r(uint16_t **output_r, uint16_t **output_opt_r,
                          int32_t num) {
    TEST_ALLIGN_MALLOC(
        uint16_t *, *output_r, sizeof(uint16_t) * MAX_SB_SIZE * num);
    TEST_ALLIGN_MALLOC(
        uint16_t *, *output_opt_r, sizeof(uint16_t) * MAX_SB_SIZE * num);
    eb_buf_random_u16(*output_r, MAX_SB_SIZE * num);
    memcpy(*output_opt_r, *output_r, sizeof(uint16_t) * MAX_SB_SIZE * num);
}

static void init_output_w(uint16_t **output_w, uint16_t **output_opt_w,
                          int32_t num) {
    TEST_ALLIGN_MALLOC(
        uint16_t *, *output_w, sizeof(uint16_t) * MAX_SB_SIZE * num);
    TEST_ALLIGN_MALLOC(
        uint16_t *, *output_opt_w, sizeof(uint16_t) * MAX_SB_SIZE * num);
            ...
            for (int x = 0; x < 2; x++) {   // Bit Depth
                ...
                case 0:  // 16x16
                    for (int j = 0; j < 16; j++) {
                        ...
                        ASSERT(eb_buf_compare_s32(coeff,
                                                  coeff_opt,
                                                  MAX_SB_SIZE * stride) == 1);
                        init_output_r(&output_r, &output_opt_r, stride);
                        init_output_w(&output_w, &output_opt_w, stride);
                        av1_inv_txfm_highbd_func_ptr_array_base[loop](
                            coeff,
                            output_r,
                            stride,
                            output_w,
                            ...
                            (TxType)txsize_16[j],
                            bd[x]);
                        av1_inv_txfm_highbd_func_ptr_array_opt[loop](
                            coeff,
                            output_opt_r,
                            stride,
                            output_opt_w,
                        ...
                        EXPECT_EQ(
                            eb_buf_compare_u16(
                                output_w, output_opt_w, MAX_SB_SIZE * stride),
                            1);
            ...
            for (int x = 0; x < 2; x++) {   // Bit Depth
                ...
                case 0:  // 32x16
                    for (int j = 0; j < 2; j++) {
                        ...
                        init_output_r(&output_r, &output_opt_r, stride);
                        init_output_w(&output_w, &output_opt_w, stride);
                        av1_inv_txfm_highbd_rect_func_ptr_array_base[loop](
                            coeff,
                            output_r,
                            stride,
                            output_w,
                            stride,
                            (TxType)txsize_32[j],
                            (TxSize)rect_types[loop],
                            0,
                            bd[x]);
                        av1_inv_txfm_highbd_rect_func_ptr_array_opt[loop](
                            coeff,
                            output_opt_r,
                            stride,
                            output_opt_w,
