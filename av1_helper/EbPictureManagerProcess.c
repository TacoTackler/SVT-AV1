...
void* picture_manager_kernel(void *input_ptr)
{
    ...
    for (;;) {
        ...
        case EB_PIC_INPUT:
			//循环picture_manager_reorder_queue，将输入图片加入manager-queue，
			//填充inputEntryPtr，加入input-queue
			//填充referenceEntryPtr，加入ref-queue，用pred-struct对应位置ptr信息填充
            ...
            else {//初始picture_manager_reorder_queue队列为空，将queueEntryPtr指向传入的inputptr，填充进queue
			...
			//循环Picture Manager Reorder Queue
			//初始picture_manager_reorder_queue为空，随着越多图片进入处理，来填充queue。
            while (queueEntryPtr->parent_pcs_wrapper_ptr != EB_NULL) {
				...
				//根据当前图片在pred-struct中的位置，拿到对应位置的ptr
                ...
                if (!picture_control_set_ptr->is_overlay) {
                    ...
                    CHECK_REPORT_ERROR(
                        ...
                        encode_context_ptr->app_callback_ptr,
                        EB_ENC_PM_ERROR5);
					// 创建并填充referenceEntryPtr，使用当前帧对应于预测结构相应位置的predPositionPtr中的信息
                ...
                if (inputEntryPtr->input_object_ptr != EB_NULL) {
                    ...
                    for (refIdx = 0; refIdx < entryPictureControlSetPtr->ref_list0_count; ++refIdx) {
                        ...
                        {   //overlay frame的ref-pic设置为其所属的帧
                            ...
                            //    0;
							//判断当前帧所找到的这个参考帧是否已进入manager-queue
                            availabilityFlag =
