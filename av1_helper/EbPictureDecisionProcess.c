...
EbErrorType generate_picture_window_split(
    ...
    // Loop over all mini GOPs
	// 在MiniGopStatsArray[mini_gop_index]中找到满足当前pre-assign-buffer中帧个数的gop类型，填充mini-gop。
	// 但由于MiniGopStatsArray[mini_gop_index]的值已经确定，
	//      * 则pre-assign-buffer的帧数为16时，用4层结构的项填充mini-gop
	//      * 其他时候，用3层结构填充mini-gop
	// 其中：对于条件(MiniGopIndex)->end_index < pre_assignment_buffer_count
	//      * 持续在MiniGopStatsArray中搜索所需结构项，填充mini-gop，直到gop-end大于pre_assignment_buffer_count
	//        此时，pre-assign-buffer成功被分割为n个gop
	//      * 剩下不够一个gop的帧，由后续调用函数将之由一个3层结构表示。
    MiniGopIndex = 0;
...
*
* pre-assign-buffer剩下的帧再组成一个mini-gop
***************************************************************************************************/
...
void* picture_decision_kernel(void *input_ptr)
{
    ...
    for (;;) {
		...
		// 首帧以及原始帧is_overlay都为0，复制帧为1，因而进入decision-queue的必为原始帧
		//将图片按照display order放入decision-queue
        if (!picture_control_set_ptr->is_overlay ) {
			//当前帧与decision-queue的head帧picture num相差多少，就存到对应的decision-queue位置
		...
		//遍历decision-queue，首项必须存在，才继续执行。
		//首项不在，则退出。
		//此部分中：
		//  * 从decision-queue首项开始，填充ParentPcsWindow，要填满ParentPcsWindow【0-7】，windowAvail才为真。
		//  * 即当decision-queue中有6项时，windowAvail才为真。
		//  * windowAvail为真后，判断这些帧中是否有scene-change，设置flag
		//  * 当接受完所有图片后，framePasseThru = EB_TRUE。
		//当首项在时，复制帧也能够进入此部分，但并未做任何事。
        while (queueEntryPtr->parent_pcs_wrapper_ptr != EB_NULL) {
			...
			//填充ParentPcsWindow
			//从decision-queue的首项开始，要填满ParentPcsWindow【0-7】，windowAvail才为真。
			...
			//framePass:当end-seq为真时，才为真。
			//windowAvail：当ParentPcsWindow【0-7】全部填满，才为真
			//任意一种满足才能继续向下执行，否则退出。
			//即，当decision-queue中有足够图片时，或者图片已经全部加载进来时，才继续向下执行。
			//由于不满足条件时，直接退出。则第一次进入的必为decision-queue首帧。
			//因而填充pre-assign-buffer的前n帧必为decision-queue的前n帧
			//在此部分：
			//   * 为I帧设置intra相关。
			//   * 
            if (windowAvail == EB_TRUE || framePasseThru == EB_TRUE)
            {
				...
				// 当满足如下条件任一项时，创建GOP，为gop中每一帧从ref-queue选择其list0,1中的对应参考帧
				//  1.当pre-assign-buffer中有IDR/CRA帧时
				//        更换pred-struct/scene change/更新ref-queue depend count等
				//  2.当pre-assign-buffer中帧数满足pred-struct要求时
				//        为每一帧在pred-struct中找到对应帧，并从ref-queue选择其list0,1中的对应参考帧
				//  3.当pred-struct是延迟P/B帧时
				//        只参考display order的前一帧
				//  4.当eos-flag为真时 ？
                    ...
                    (picture_control_set_ptr->pred_structure == EB_PRED_LOW_DELAY_B))
                {
                    ...
                    //generate GOPs
					//根据pre-assign-buffer中帧数，分成n个某种层次机构的mini-gop。
                    {
					...
					//generate pred-struct
					//根据上面确定的分层结构，设置每个GOP中每一帧的pred-struct ptr
                    GenerateMiniGopRps(
                        context_ptr,
                        encode_context_ptr);

                    // Loop over Mini GOPs
					// 此时pre-assign-buffer中的帧已经分进了mini-gops
                    for (mini_gop_index = 0; mini_gop_index < context_ptr->total_number_of_mini_gops; ++mini_gop_index) {
                        ...
                        for (pictureIndex = context_ptr->mini_gop_start_index[mini_gop_index]; pictureIndex <= context_ptr->mini_gop_end_index[mini_gop_index]; ++pictureIndex) {
							...
							//设置pred-struct-position
							//   *如果有I帧，....
							//   *有分层结构变化，。。。
							//   *其他情况，pred-struct-position+1。
							//    在整个pre-assign-buffer循环中每处理完一帧，++
                            // If mini GOP switch, reset position
							//如果gop的分层结构跟上个gop不同，当前pred_struct_position设置为init起始位置
                            ...
                            if (sequence_control_set_ptr->static_config.enable_overlays == EB_TRUE) {
                                ...
                                // release the overlay PCS for non alt ref pictures. First picture does not have overlay PCS（多余
                                //此处release 不需要的复制帧的overlay-ptr
							...
							//loop0：原始帧进ref-queue，并设置ME所需的参数
							//loop1：原始帧的复制帧，进一步填充参数，包括ME所需参数，但是不进ref-queue
							...
							//生成alt-ref
							//  *建立当前帧的temporal-filter-list
							//  *post当前帧，给ME-kernel，做temporal-filter，生成alt-ref，替代当前帧。
							//  *直到替代完成返回信号，才继续向下执行。
                            if( sequence_control_set_ptr->enable_altrefs == EB_TRUE &&
								...
								// 计算当前帧与以前帧的差异的累计直方图。
								// 图片离得越近，两张图差异越小。
								// 随着循环，图片距离越近，差异越小。当差异减小到首次满足阙值时，break。
								// 此时的图片到当前帧之间的图片，即为当前帧的past_altref_nframes。
                        ...
                        for (pictureIndex = context_ptr->mini_gop_start_index[mini_gop_index]; pictureIndex <= context_ptr->mini_gop_end_index[mini_gop_index] + has_overlay; ++pictureIndex) {
                                    ...
                                    if (picture_control_set_ptr->ref_list0_count) {
                                        ...
                                        if(picture_control_set_ptr->is_overlay){
                                            ...
                                            paReferenceQueueIndex = (uint32_t)CIRCULAR_ADD(
                                                ...
                                                PICTURE_DECISION_PA_REFERENCE_QUEUE_MAX_DEPTH);                                                                                             // Max
											//找到ref-queue中满足list0中与当前帧相对距离的对应项，即为当前帧对应的参考帧
                                            paReferenceEntryPtr = encode_context_ptr->picture_decision_pa_reference_queue[paReferenceQueueIndex];
							...
							//遍历至当前gop最后一帧，调整decode base number，为后面一个gop继续设置。
							...
							//遍历至pre-assign-buffer最后一帧，重置pre-assign-buffer count。
